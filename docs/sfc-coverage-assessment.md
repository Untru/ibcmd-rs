...
| Формы | Существенно частично | `prepare_form_body_row` читает `Ext/Form.xml` и патчит верхние `WindowOpeningMode`/`Group`, имя уже существующей `AutoCommandBar`, обработчики уже существующих `Events`, существующие `Attributes` (`name`, `MainAttribute`, часть `DynamicList` settings), существующие `Commands`, существующий `CommandInterface/NavigationPanel` (`CommandGroup`, `Index`, `DefaultVisible`, `Visible/xr:Common`) и существующие `ChildItems` (`name`, `Title`, часть `Button/CommandName`) в base blob; если есть `Ext/Form/Module.bsl`, дополнительно заменяет текст модуля. Полная компиляция `Ext/Form.xml` пока не реализована. |
