...
| `cargo test` | 315 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Формы | Существенно частично | `prepare_form_body_row` читает `Ext/Form.xml` и патчит верхние `WindowOpeningMode`/`Group` в base blob; если есть `Ext/Form/Module.bsl`, дополнительно заменяет текст модуля. Полная компиляция `Ext/Form.xml` пока не реализована. |
...
| Управляемые формы | 13 044 `Ext/Form.xml`, 590 МБ XML; 12 216 form modules | Низкая/средняя для частичной выгрузки: есть `WindowOpeningMode`, `Group`, `AutoCommandBar`, часть `Events`, базовые `Attributes`/`Commands`/`CommandInterface`/`ChildItems`, module/assets | Низкая/частичная: `Ext/Form.xml` патчит только top-level `WindowOpeningMode`/`Group`, `Ext/Form/Module.bsl` заменяет модуль; полный `Ext/Form.xml` не компилируется | Полный compiler/decompiler всех `ChildItems`, `Parameters`, все типы реквизитов, `ListSettings`, events и вложенных form assets |
...
| Агент parity-аудита | source coverage, SQL dry-run, batch accounting, dump/load diff | Есть `audit-source-load-coverage` source-side baseline без SQL: 55 789 stage entry files, 59 968 potentially stageable body files, 13 344 known uncovered. SQL-backed `mssql-audit-source-parity` уже прогнан на копии `sfc` по справочнику `Валюты` и смешанному smoke-срезу; actual `mssql-stage-source-objects --path-prefix Catalogs/Валюты` записал 19-row staged set в `ConfigSave` одноразовой SQL-копии. Следующий критерий - применить staged set через 1С/ibcmd и сделать post-dump/post-compare. |
