...
| `audit-source-load-coverage D:\УХА\sfc` | 140 709 файлов / 9 593 572 857 bytes; stage entry files 55 789, из них metadata XML 51 085 и common module XML 4 704; potentially stageable body files 59 968; module files 29 818, supported module files 29 814; supported Ext body files 30 154; known uncovered 13 344 files / 592 763 561 bytes | Общий source-side baseline для любой конфигурации: показывает, какие исходники текущий loader вообще рассматривает как вход, и какие известные блоки остаются вне загрузки до SQL. Основной uncovered блок - 13 044 полных `Ext/Form.xml` / 590 173 726 bytes; 12 216 из них имеют stageable module-only путь, 828 не имеют даже `Ext/Form/Module.bsl`; дополнительно 295 non-module files внутри `Ext/Form` / 1 725 563 bytes. |
| `mssql-audit-source-parity --database ibcmd_rs_sfc_20260626_v85 --source-root D:\УХА\sfc --path-prefix Catalogs/Валюты` | 5 metadata XML выбрано, 4 metadata objects подготовлено, 7 body rows, 11 total Config rows; `prepare_failure_summary`: `missing_config_row=1`; failure: `Config row not found: 7aadbb67-f93e-43bb-9f53-f14d2c2a347a.5`; `version_patch_category`: `unsupported_versions_shape`; `version_patch_error`: `versions entry not found: root`; 1 batch, expected_total_rows 14 | SQL-backed dry-run на реальной копии базы `sfc`: команда работает как точечный parity-аудит без записи в SQL, показывает подготовленные blob-ы, batch accounting и конкретные машинно-классифицированные несовпадения с baseline `Config`. |
| `mssql-audit-source-parity --database ibcmd_rs_sfc_20260626_v85 --source-root D:\УХА\sfc --path-prefix CommonModules/CRMЛокализация --path-prefix Roles/АдминистраторПроцесса --path-prefix Enums/ВариантыЗагрузкиРабочихЦентров --path-prefix CommonPictures/AppStore` | 3 metadata XML и 1 common module XML выбрано; 3 metadata objects и 1 common module подготовлены; 2 body rows, 7 total Config rows; prepare failures 0; `version_patch_category`: `unsupported_versions_shape`; 1 batch, expected_total_rows 10 | Смешанный smoke-аудит по разным семействам показал, что текущий loader может подготовить простой срез metadata/common module/body rows против SQL baseline; общий blocker для фиксации staged changes сейчас `versions` blob shape. |
| `cargo test` | 305 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Агент parity-аудита | source coverage, SQL dry-run, batch accounting, dump/load diff | Есть `audit-source-load-coverage` source-side baseline без SQL: 55 789 stage entry files, 59 968 potentially stageable body files, 13 344 known uncovered. SQL-backed `mssql-audit-source-parity` уже прогнан на копии `sfc` по справочнику `Валюты` и смешанному smoke-срезу: умеет считать `prepare_failure_summary`, выявил `missing_config_row` и `unsupported_versions_shape`. Следующий критерий - исправить patch `versions` для формы blob-а этой базы и расширять прогон по семействам объектов. |
