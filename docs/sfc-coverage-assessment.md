...
| XML -> SQL обновление совместимой базы | Средняя готовность на поддержанных классах | `audit-source-load-coverage` видит 55 789 stage entry files и 59 973 potentially stageable body files; smoke apply по `Catalogs/Валюты` прошел через `ConfigSave` и штатный `ibcmd apply` | Расширить SQL-backed parity/apply на несколько семейств объектов и проверить batch accounting на длинной загрузке |
...
8. Object-level `AdditionalIndexes.xml`. В `sfc` встречаются такие body-файлы у отдельных регистров/документов, например `AccumulationRegister.*.AdditionalIndexes` и `Document.*.AdditionalIndexes`; это отдельный фронт от корневых assets конфигурации.
...
| Корневые assets конфигурации | несколько известных файлов `Ext/*` | Средняя/высокая: `Splash`, `ParentConfigurations`, `HomePageWorkArea`, `MobileClientSignature`, `MainSectionCommandInterface`, `ClientApplicationInterface`, `MainSectionPicture`, `StandaloneConfigurationContent` маршрутизируются | Средняя/высокая: те же root assets готовятся обратно как picture/binary/raw-deflated/command-interface rows | SQL e2e на реальной базе и отдельный object-level фронт `AdditionalIndexes.xml` |
