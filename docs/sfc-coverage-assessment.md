...
| `cargo test` | 313 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Управляемые формы | 13 044 `Ext/Form.xml`, 590 МБ XML; 12 216 form modules | Низкая/средняя для частичной выгрузки: есть `WindowOpeningMode`, `Group`, `AutoCommandBar`, часть `Events`, базовые `Attributes`/`Commands`/`CommandInterface`, module/assets | Низкая для полной формы: грузится только `Ext/Form/Module.bsl`, полный `Ext/Form.xml` не компилируется | Полный compiler/decompiler `ChildItems`, `Parameters`, все типы реквизитов, `ListSettings`, events и вложенных form assets |
