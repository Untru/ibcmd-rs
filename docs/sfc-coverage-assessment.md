...
| `cargo test` | 319 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Формы | Существенно частично | `prepare_form_body_row` читает `Ext/Form.xml` и патчит верхние `WindowOpeningMode`/`Group`, имя уже существующей `AutoCommandBar` и обработчики уже существующих `Events` в base blob; если есть `Ext/Form/Module.bsl`, дополнительно заменяет текст модуля. Полная компиляция `Ext/Form.xml` пока не реализована. |
