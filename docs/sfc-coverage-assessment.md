...
| Формы | Существенно частично | `prepare_form_body_row` читает `Ext/Form.xml` и патчит верхние `WindowOpeningMode`/`Group`, имя уже существующей `AutoCommandBar`, обработчики уже существующих `Events`, существующие `Attributes` (`name`, `MainAttribute`, часть `DynamicList` settings: `ManualQuery`, `DynamicDataRead`, `QueryText`, `MainTable` при наличии `source_root`), существующие `Commands` (`Title`, `ToolTip`, `Action`, `CurrentRowUse`, `FunctionalOptions` при наличии `source_root`), существующий `CommandInterface/NavigationPanel` (`Command`, `CommandGroup`, `Index`, `DefaultVisible`, `Visible/xr:Common`) и существующие `ChildItems` (`name`, `Title`, часть `Button/CommandName`, включая внешние `Kind.Object.Command.Name` при наличии `source_root`, `Button/DataPath` для `Items.<Table>.CurrentData.<Field>` по `Ссылка` и найденным колонкам таблицы) в base blob; если есть `Ext/Form/Module.bsl`, дополнительно заменяет текст модуля. Полная компиляция `Ext/Form.xml` пока не реализована. |
