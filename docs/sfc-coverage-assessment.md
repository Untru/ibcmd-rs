...
| `mssql-audit-source-parity --database ibcmd_rs_sfc_20260626_v85 --source-root D:\УХА\sfc --path-prefix Catalogs/Валюты` | 5 metadata XML выбрано, 4 metadata objects подготовлено, 7 body rows, 11 total Config rows; `prepare_failure_summary`: `missing_config_row=1`; failure: `Config row not found: 7aadbb67-f93e-43bb-9f53-f14d2c2a347a.5`; `version_patch_error`: empty; 12 `version_replacements`; 1 batch, expected_total_rows 14 | SQL-backed dry-run на реальной копии базы `sfc`: `versions` patch теперь проходит для большого flat-list shape без `root/version/versions` entries; оставшаяся ошибка - конкретный отсутствующий baseline `Config` blob. |
...
| `cargo test` | 307 passed | Unit-покрытие текущего кода стабильно после изменений. |
