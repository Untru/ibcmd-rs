...
| `audit-source-load-coverage D:\УХА\sfc` | 140 709 файлов / 9 593 572 857 bytes; stage entry files 55 789, из них metadata XML 51 085 и common module XML 4 704; potentially stageable body files 59 969; module files 29 818, supported module files 29 814; supported Ext body files 30 155; known uncovered 13 343 files / 592 763 517 bytes | Общий source-side baseline для любой конфигурации: показывает, какие исходники текущий loader вообще рассматривает как вход, и какие известные блоки остаются вне загрузки до SQL. Основной uncovered блок - 13 044 полных `Ext/Form.xml` / 590 173 726 bytes; 12 216 из них имеют stageable module-only путь, 828 не имеют даже `Ext/Form/Module.bsl`; дополнительно 295 non-module files внутри `Ext/Form` / 1 725 563 bytes; из корневых configuration assets закрыт `MobileClientSignature.bin`. |
...
| `cargo test` | 318 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Формы | Существенно частично | `prepare_form_body_row` читает `Ext/Form.xml` и патчит верхние `WindowOpeningMode`/`Group` плюс обработчики уже существующих `Events` в base blob; если есть `Ext/Form/Module.bsl`, дополнительно заменяет текст модуля. Полная компиляция `Ext/Form.xml` пока не реализована. |
...
8. Корневые assets конфигурации. В `sfc` встречаются `AdditionalIndexes.xml`, `StandaloneConfigurationContent.bin`, `MainSectionCommandInterface.xml`, `ClientApplicationInterface.xml`, `HomePageWorkArea.xml`; текущий staging покрывает часть конфигурационных assets (`Splash`, `ParentConfigurations`, `MainSectionPicture`, `MobileClientSignature`).
...
| Управляемые формы | 13 044 `Ext/Form.xml`, 590 МБ XML; 12 216 form modules | Низкая/средняя для частичной выгрузки: есть `WindowOpeningMode`, `Group`, `AutoCommandBar`, часть `Events`, базовые `Attributes`/`Commands`/`CommandInterface`/`ChildItems`, module/assets | Низкая/частичная: `Ext/Form.xml` патчит top-level `WindowOpeningMode`/`Group` и обработчики уже существующих `Events`, `Ext/Form/Module.bsl` заменяет модуль; полный `Ext/Form.xml` не компилируется | Полный compiler/decompiler всех `ChildItems`, `Parameters`, все типы реквизитов, `ListSettings`, добавление/удаление events и вложенных form assets |
...
| Корневые assets конфигурации | несколько известных файлов `Ext/*` | Частично | Частично | Закрыть `AdditionalIndexes.xml`, `StandaloneConfigurationContent.bin`, `MainSectionCommandInterface.xml`, `ClientApplicationInterface.xml`, `HomePageWorkArea.xml`; `MobileClientSignature.bin` уже маршрутизируется как deflated raw body |
...
| Агент parity-аудита | source coverage, SQL dry-run, batch accounting, dump/load diff | Есть `audit-source-load-coverage` source-side baseline без SQL: 55 789 stage entry files, 59 969 potentially stageable body files, 13 343 known uncovered. SQL-backed `mssql-audit-source-parity` уже прогнан на копии `sfc` по справочнику `Валюты`, форме `Валюты.ФормаСписка` и смешанному smoke-срезу; scoped scan по `path-prefix` больше не обходит весь `sfc`. Actual `mssql-stage-source-objects --path-prefix Catalogs/Валюты` записал 19-row staged set в `ConfigSave` одноразовой SQL-копии. Следующий критерий - применить staged set через 1С/ibcmd и сделать post-dump/post-compare. |
