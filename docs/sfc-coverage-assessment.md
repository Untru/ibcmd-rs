...
| `cargo test` | 321 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Агент форм | compile/decompile `Ext/Form.xml`, form item assets | Есть `audit-form-sources` baseline: 13 044 форм, 590 МБ XML, 12 216 stageable-by-module, 828 без stageable body, 295 ignored non-module `Ext/Form` files. Обратный compile уже патчит верхние свойства, существующую `AutoCommandBar`, существующие `Events`, существующие `Attributes` и существующие `Commands`. Следующий критерий - расширить базовый decompile `ChildItems` на больше типов элементов/свойств и продолжить compile минимального набора верхних секций (`CommandInterface`, `ChildItems`) хотя бы на малой форме, затем расширять до `sfc`. |
