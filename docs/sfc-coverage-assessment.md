...
| Повторный `mssql-dump-config --database ibcmd_rs_sfc_stage_valyuty_20260627 --file-name 7aadbb67-f93e-43bb-9f53-f14d2c2a347a --file-name 5f91b00f-d8fc-4d63-8486-66339357ab22 --file-name 097b7cbf-cd63-4213-96ae-fc88d1be24d6 --file-name b34b580f-305b-4e22-b601-cfcf4dabaae0 --file-name 9e3fce60-09bb-4d21-af76-3ac4e1a12396 --extract-metadata-xml` после разбора `ChildItems` формы | Selected dump с владельцами всех внешних команд выгрузил валидный `Form.xml`; `form-info` видит дерево элементов формы: `ГруппаПользовательскихНастроек`, `КоманднаяПанель`, popup `Создать`, кнопки локальных/стандартных/внешних команд, popup `ГруппаЗагрузитьКурсыВалют`, таблицу `Валюты -> Список`, колонки `НаименованиеПолное`, `Код`, `Наименование`, `ОтносительныйКурс`, `Ссылка`. Восстановлены `CommandName` для локальных команд, стандартных `Create/Help` и внешних команд при наличии owner metadata; восстановлены `DataPath` таблицы и колонок; unit-тестом добавлен round-trip событий элементов `OnGetDataAtServer`, `OnChange`, `StartChoice` | Закрыт базовый decompile layout tree для подтвержденных элементов (`UsualGroup`, `CommandBar`, `Popup`, `ButtonGroup`, `Button`, `Table`, `InputField`, `LabelField`, search additions) и частых событий существующих элементов. Это все еще не полный `Form.xml`: не покрыты все типы элементов, расширенные свойства элементов, `ListSettings` и compile обратно в blob. |
...
| `cargo test` | 326 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Формы | Существенно частично | `prepare_form_body_row` читает `Ext/Form.xml` и патчит верхние `WindowOpeningMode`/`Group`, имя уже существующей `AutoCommandBar`, обработчики уже существующих `Events`, существующие `Attributes` (`name`, `MainAttribute`, часть `DynamicList` settings: `ManualQuery`, `DynamicDataRead`, `QueryText`, `MainTable` при наличии `source_root`, `ListSettings` для `filter`/`order`/`conditionalAppearance`/`itemsViewMode`/`itemsUserSettingID`), существующие `Commands` (`Title`, `ToolTip`, `Action`, `CurrentRowUse`, `FunctionalOptions` при наличии `source_root`), существующий `CommandInterface/NavigationPanel` (`Command`, `CommandGroup`, `Index`, `DefaultVisible`, `Visible/xr:Common`) и существующие `ChildItems` (`name`, `Title`, события, часть `Button/CommandName`, включая внешние `Kind.Object.Command.Name` при наличии `source_root`, `Button/DataPath` для `Items.<Table>.CurrentData.<Field>` по `Ссылка` и найденным колонкам таблицы) в base blob; если есть `Ext/Form/Module.bsl`, дополнительно заменяет текст модуля. Полная компиляция `Ext/Form.xml` пока не реализована. |
