...
| Forms | Существенно частично | `extract_form_body_xml` возвращает каркас `Form.xml`, распознает часть `Events` включая UUID-based события `OnOpen`/`ChoiceProcessing`/`NotificationProcessing`/`OnCreateAtServer`, извлекает `AutoCommandBar`; resolver форм поддерживает wrapped form metadata code `13` и `14`; часть вложенных item assets выгружается отдельно. Разбор `{4, layout, module, ...}` вынесен в общий `parse_form_body_blob`, который также используется при загрузочной замене модуля формы. |
