...
| Unit round-trip `Form/Attributes/Settings/ListSettings/order` на реальном shape из `Catalogs/Валюты/Forms/ФормаСписка` | Реальный body формы хранит DCS order внутри DynamicList settings как ключ `Order`: `{"#",11743ff3-2db3-4cfc-9404-90ed8209437f,{#base64:<Order XML>}}`. Добавлены тесты на выгрузку `<ListSettings><dcsset:order>` с `dcsset:item`, `field`, `orderType`, `viewMode`, `userSettingID` и на обратный patch существующего `Order` base64 payload в blob | Закрыт первый слой `ListSettings` для сортировки динамического списка. Ограничение: `filter`, `conditionalAppearance`, `itemsViewMode`, параметры, отборы и прочие DCS settings пока не компилируются/декомпилируются полностью. |
...
| Формы | Существенно частично | `prepare_form_body_row` читает `Ext/Form.xml` и патчит верхние `WindowOpeningMode`/`Group`, имя уже существующей `AutoCommandBar`, обработчики уже существующих `Events`, существующие `Attributes` (`name`, `MainAttribute`, часть `DynamicList` settings: `ManualQuery`, `DynamicDataRead`, `QueryText`, `MainTable` при наличии `source_root`, `ListSettings/order`), существующие `Commands` (`Title`, `ToolTip`, `Action`, `CurrentRowUse`, `FunctionalOptions` при наличии `source_root`), существующий `CommandInterface/NavigationPanel` (`Command`, `CommandGroup`, `Index`, `DefaultVisible`, `Visible/xr:Common`) и существующие `ChildItems` (`name`, `Title`, события, часть `Button/CommandName`, включая внешние `Kind.Object.Command.Name` при наличии `source_root`, `Button/DataPath` для `Items.<Table>.CurrentData.<Field>` по `Ссылка` и найденным колонкам таблицы) в base blob; если есть `Ext/Form/Module.bsl`, дополнительно заменяет текст модуля. Полная компиляция `Ext/Form.xml` пока не реализована. |
