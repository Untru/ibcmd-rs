...
| `mssql-audit-source-parity --database <db> --source-root D:\УХА\sfc` | Команда добавлена; на `sfc` еще не запускалась без выбранной SQL-базы baseline | Non-mutating SQL-backed dry-run: сканирует source coverage, готовит metadata/body/common module blob-ы против существующих `Config` base blob-ов, собирает prepare failures и batch plan без записи в `ConfigSave`. |
| `cargo test` | 300 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Агент parity-аудита | source coverage, SQL dry-run, batch accounting, dump/load diff | Есть `audit-source-load-coverage` source-side baseline без SQL: 55 789 stage entry files, 59 968 potentially stageable body files, 13 344 known uncovered. Добавлен SQL-backed `mssql-audit-source-parity`: вызывает подготовку blob-ов без записи и фиксирует failures/batch plan. Следующий критерий - прогон на копии SQL-базы `sfc` и разбор missing base blobs/packer failures. |
...
3. Прогнать `mssql-audit-source-parity` на копии SQL-базы `sfc` и разложить failures по типам: missing base blob, unsupported packer, broken source reference, batch/accounting issue.
