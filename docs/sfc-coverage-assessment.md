...
| `mssql-audit-source-parity --database ibcmd_rs_sfc_20260626_v85 --source-root D:\УХА\sfc --path-prefix Catalogs/Валюты` | 5 metadata XML выбрано, 4 metadata objects подготовлено, 7 body rows, 11 total Config rows; 1 prepare failure: `Config row not found: 7aadbb67-f93e-43bb-9f53-f14d2c2a347a.5`; `version_patch_error`: `versions entry not found: root`; 1 batch, expected_total_rows 14 | Первый SQL-backed dry-run на реальной копии базы `sfc`: команда работает как точечный parity-аудит без записи в SQL, показывает подготовленные blob-ы, batch accounting и конкретные несовпадения с baseline `Config`. |
| `cargo test` | 303 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Агент parity-аудита | source coverage, SQL dry-run, batch accounting, dump/load diff | Есть `audit-source-load-coverage` source-side baseline без SQL: 55 789 stage entry files, 59 968 potentially stageable body files, 13 344 known uncovered. SQL-backed `mssql-audit-source-parity` уже прогнан на копии `sfc` по `--path-prefix Catalogs/Валюты`: 11 Config rows подготовлено, выявлен 1 missing base blob и проблема patch `versions`. Следующий критерий - расширять прогон по семействам объектов и разложить failures по типам: missing base blob, unsupported packer, broken source reference, batch/accounting issue. |
