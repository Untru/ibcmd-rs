...
`D:\УХА\sfc` здесь не целевая конфигурация, а benchmark реальной большой конфигурации. По нему оценивается универсальность `ibcmd-rs`: если класс объектов закрыт на таком дереве исходников, это сильный признак, что он закрыт не только для одной базы.

На 2026-06-28 `ibcmd-rs` уже покрывает значимую часть выгрузки и staged-загрузки: модули, многие metadata XML, множество `Ext` body-файлов, табличные макеты, картинки, справку, роли и command interface. Но это еще не полный аналог `ibcmd`: загрузка из XML сейчас в основном работает как patch/staging поверх существующей SQL-базы через `ConfigSave`, а не как самостоятельный bootstrap абсолютно пустой базы.

| Вопрос | Ответ по текущему состоянию |
|---|---|
| Можно ли использовать `sfc` для оценки универсальности? | Да. Это большой эталонный набор: 140 709 файлов, 25 977 объектов в `Configuration.xml`, 29 818 `.bsl`, 13 044 `Ext/Form.xml`, 15 769 `Ext/Template.xml`, 2 114 `Rights.xml`. |
| Может ли `ibcmd-rs` полноценно выгрузить произвольную SQL-базу в XML? | Пока нет гарантии. Native `mssql-dump-config` умеет много типов тел, но metadata XML частично минимальные, а полные формы остаются главным разрывом. |
| Может ли `ibcmd-rs` загрузить XML-исходники в новую пустую SQL-базу? | Пока нет. Текущий XML -> SQL путь зависит от существующих base blob-ов в SQL и патчит их. |
| Что уже наиболее близко к готовому? | BSL-модули, `SpreadsheetDocument` pack/semantic round-trip, многие template/body assets, картинки, help, schedule/content/flowchart/predefined bodies. |
| Главный риск для полноценного аналога `ibcmd` | Полный compile/decompile управляемых форм и bootstrap всех metadata/body rows без существующей базы-основы. |
...
## Матрица полноценности на эталоне `sfc`

Ниже `D:\УХА\sfc` используется не как специальная цель разработки, а как контрольная выборка сложности. Статус "закрыто на `sfc`" означает, что класс объектов прошел репрезентативный аудит на большой реальной конфигурации и поэтому может считаться кандидатом на универсальную поддержку; статус "не закрыто" означает риск для любой похожей ERP/КА/УХА-конфигурации, а не только для этой папки.

| Проверяемая возможность | Текущий статус | Что доказывает `sfc` | Что еще нужно до аналога `ibcmd` |
|---|---|---|---|
| Native SQL -> XML выгрузка всей конфигурации | Частично | `mssql-dump-config` уже умеет строить layout исходников из SQL для модулей, многих metadata XML и большого набора `Ext` bodies | Полный structural diff `native dump -> compare with sfc` по всей конфигурации, особенно формы и расширенные metadata XML |
| XML -> SQL обновление совместимой базы | Средняя готовность на поддержанных классах | `audit-source-load-coverage` видит 55 789 stage entry files и 60 276 potentially stageable body files; smoke apply по `Catalogs/Валюты` прошел через `ConfigSave` и штатный `ibcmd apply` | Расширить SQL-backed parity/apply на несколько семейств объектов и проверить batch accounting на длинной загрузке |
| XML -> новая пустая SQL-база | Низкая / не доказана | Текущий путь почти всегда берет active `Config` rows как base blobs и патчит их | Нужен отдельный bootstrap compiler: создание всех `_Config`/metadata/body/versions rows без существующей базы-основы |
| Полный round-trip `dump -> load -> apply -> dump -> diff` | Не закрыт | Для `SpreadsheetDocument` dry-run semantic round-trip закрыт 14 046/14 046; для одного реального каталога есть SQL apply smoke | Нужен интеграционный harness на небольшой базе, затем на срезах `sfc`, затем на полной конфигурации |
| Формы `Ext/Form.xml` | Главный blocker | Все 13 044 формы валидно парсятся как XML, но 590 173 726 bytes полного `Form.xml` остаются uncovered; loader сейчас в основном патчит существующий form blob | Полный compile/decompile форм: создание/удаление attributes/parameters/events/commands/items, все типы элементов, все ссылки, assets и настройки |
| Макеты `SpreadsheetDocument` | Высокая dry-run готовность | 14 046/14 046 `Template.xml` пакуются, извлекаются и совпадают в semantic round-trip | SQL e2e `dump -> stage/load -> apply -> dump -> diff` и stress-test остальных TemplateType |
| Роли, help, картинки, command interface | Частично/средне | Есть маршруты выгрузки/загрузки и unit-тесты на ключевые форматы | Массовый round-trip на всех 2 114 `Rights.xml`, 6 333 `Help.xml`, 3 247 `Picture.xml`, 331 `CommandInterface.xml` |
...
| `Ext/Form.xml` | 13 044 | Добавлен `audit-form-sources`: все 13 044 XML валидно парсятся как XML-структура; 12 216 форм имеют `Ext/Form/Module.bsl`, 12 218 имеют вложенные файлы в `Ext/Form`. Все 13 044 `Form.xml` / 590 173 726 XML bytes остаются неподдержанной полной структурой формы. Выгрузка сейчас формирует каркас формы, распознанные `Events` включая document lifecycle (`AfterWrite`, `BeforeWrite`, `OnWriteAtServer`, `OnLoadDataFromSettingsAtServer`, `FillCheckProcessingAtServer`), settings/user-variant events, неизвестные UUID-события без потери идентификатора, top-level `Title`, `Width`, `Height`, `AutoTitle=false`, `CommandBarLocation`, top-level `AutoCommandBar` с `HorizontalAlign` и `Autofill=false`, верхние свойства, базовые `Attributes` для `DynamicList`, базовые `Commands`, базовый `CommandInterface/NavigationPanel`, базовое дерево `ChildItems` для подтвержденных элементов layout, частые события элементов (`OnGetDataAtServer`, `OnChange`, `StartChoice`, `ValueChoice`, `AutoComplete`, `Click`, `Clearing` и др.) и распознанные вложенные item assets. Загрузка использует `Ext/Form.xml` для патча top-level `Title`, `Width`, `Height`, `AutoTitle`, верхних `WindowOpeningMode`/`Group`, `CommandBarLocation`, имени, `HorizontalAlign` и `Autofill` уже существующей top-level `AutoCommandBar`, обработчиков уже существующих `Events`, включая UUID-based события по точному идентификатору, существующих `Attributes`, существующих `Commands` включая `FunctionalOptions` при наличии `source_root`, части существующего `CommandInterface/NavigationPanel` и части существующих `ChildItems` включая события в base blob, дополнительно умеет создать простой top-level `Button` в layout-list `{59,count,...}`, заменяет `Ext/Form/Module.bsl` и распознанные картинки элементов из `Ext/Form/Items`; полный compile структуры формы пока не реализован. |
...
| `*.png` | 1 497 | Встречаются в assets форм/справки; вложенные form item assets выгружаются и загружаются обратно для распознанных `Picture`/`RowsPicture`/`ValuesPicture`/`HeaderPicture`. |
...
Форматы общих картинок по `xr:Abs` в `Picture.xml`: `zip` 1 587, `png` 928, `svg` 550, `bmp` 151, `gif` 17, `jpg` 11, `ico` 3. Все ссылки из `Picture.xml` нашли физический файл в `Ext/Picture/...`. Детекторы форматов для picture/form item assets явно покрывают `zip/png/svg/bmp/gif/jpg/ico`; остается подтвердить SQL e2e round-trip на всех форматах.
...
| Повторный `audit-source-load-coverage D:\УХА\sfc` на 2026-06-28 | Результат текущего кода совпадает с baseline: 140 709 файлов / 9 593 572 857 bytes; stage entry files 55 789, из них metadata XML 51 085 и common module XML 4 704; potentially stageable body files 59 970; module files 29 818, supported module files 29 814; supported Ext body files 30 156; known uncovered 13 342 files / 592 676 841 bytes | Подтверждает, что `sfc` используется как стабильный эталонный корпус для оценки универсальности загрузчика: основной known-uncovered класс по-прежнему полный `Ext/Form.xml`, а не конкретный объект `Валюты` или другая частная конфигурационная сущность. |
| `audit-source-load-coverage D:\УХА\sfc` после маршрутизации root configuration assets `.8/.b/.f` | 140 709 файлов / 9 593 572 857 bytes; stage entry files 55 789, из них metadata XML 51 085 и common module XML 4 704; potentially stageable body files 59 973; module files 29 818, supported module files 29 814; supported Ext body files 30 159; known uncovered 13 339 files / 591 899 289 bytes | Закрыты корневые assets `Ext/HomePageWorkArea.xml`, `Ext/ClientApplicationInterface.xml`, `Ext/StandaloneConfigurationContent.bin`: native выгрузка распаковывает suffix `.8/.b/.f`, staging грузит их обратно как raw-deflated body rows. Основной known-uncovered блок теперь снова только полные формы и non-module файлы внутри `Ext/Form`; `AdditionalIndexes.xml` остается отдельным object-level body фронтом. |
| `audit-source-load-coverage D:\УХА\sfc` после маршрутизации object-level `AdditionalIndexes.xml` | 140 709 файлов / 9 593 572 857 bytes; stage entry files 55 789, из них metadata XML 51 085 и common module XML 4 704; potentially stageable body files 59 976; module files 29 818, supported module files 29 814; supported Ext body files 30 162; known uncovered 13 339 files / 591 899 289 bytes | Закрыты три подтвержденных body-файла `AdditionalIndexes.xml`: `Document.*.3` и `AccumulationRegister.*.4`. Native выгрузка распаковывает их в `Ext/AdditionalIndexes.xml`, staging пакует обратно как raw-deflated body rows. `known_uncovered` не меняется, потому что эти файлы раньше были не классифицированы как известный blocker, а просто не входили в supported body coverage. |
| `audit-source-load-coverage D:\УХА\sfc` после маршрутизации root `Ext/CommandInterface.xml` и root application modules | 140 709 файлов / 9 593 572 857 bytes; stage entry files 55 789, из них metadata XML 51 085 и common module XML 4 704; potentially stageable body files 59 981; module files 29 818, supported module files 29 818; supported Ext body files 30 163; known uncovered 13 339 files / 591 899 289 bytes | Закрыт корневой `Configuration.*.a` -> `Ext/CommandInterface.xml` в native выгрузке и staging-загрузке. Source-side audit теперь также учитывает 4 корневых модуля конфигурации (`OrdinaryApplicationModule`, `ExternalConnectionModule`, `ManagedApplicationModule`, `SessionModule`) как поддержанные, что соответствует уже существующему SQL loader/dump route. |
| `audit-source-load-coverage D:\УХА\sfc` после маршрутизации form item assets | 140 709 файлов / 9 593 572 857 bytes; stage entry files 55 789, из них metadata XML 51 085 и common module XML 4 704; potentially stageable body files 60 276; module files 29 818, supported module files 29 818; supported Ext body files 30 458; known uncovered 13 044 files / 590 173 726 bytes; ignored form Ext files 0 | Закрыты 295 вложенных файлов `Ext/Form/Items` (`png/bmp/gif/svg/jpg`) как симметричный route: native выгрузка выносит картинки элементов формы, staging загрузка подставляет их обратно в base64 payload form blob. Единственный известный uncovered блок source-side теперь полный `Ext/Form.xml`, а не вложенные assets. |
...
| Повторный `mssql-dump-config --database ibcmd_rs_sfc_stage_valyuty_20260627 --file-name 7aadbb67-f93e-43bb-9f53-f14d2c2a347a --file-name 5f91b00f-d8fc-4d63-8486-66339357ab22 --extract-metadata-xml` после извлечения верхних свойств формы | `Catalogs/Валюты/Forms/ФормаСписка/Ext/Form.xml` теперь содержит `WindowOpeningMode=DontBlock`, `Group=Vertical`, `AutoCommandBar`, события `ChoiceProcessing`, `NotificationProcessing`, `OnCreateAtServer`; дополнительно unit-тестами покрыты top-level события документных форм `AfterWrite`, `BeforeWrite`, `OnWriteAtServer`, `OnLoadDataFromSettingsAtServer`, `FillCheckProcessingAtServer`; тесты добавлены для `WindowOpeningMode` codes `0/1/2` и подтвержденных `Group` mappings `Vertical`/`Horizontal`/`AlwaysHorizontal` | Еще один шаг к native decompile формы. `Attributes`, `ChildItems`, команды и полное дерево элементов пока не реконструируются, поэтому это не полная совместимость с `ibcmd export`. |
| Повторный `mssql-dump-config --database ibcmd_rs_sfc_stage_valyuty_20260627 --file-name 7aadbb67-f93e-43bb-9f53-f14d2c2a347a --file-name 5f91b00f-d8fc-4d63-8486-66339357ab22 --extract-metadata-xml` после разбора tail-секций формы | `Catalogs/Валюты/Forms/ФормаСписка/Ext/Form.xml` теперь дополнительно содержит `Attributes` с `Список: DynamicList`, `UseAlways` по `Список.Наименование`/`Список.Ссылка`, `QueryText`, `MainTable=Catalog.Валюты`, а также команды `ПодборИзКлассификатора` и `ЗагрузитьКурсыВалют` с `Title`, `ToolTip`, `Action`, `CurrentRowUse`. `form-info` успешно распознает выгруженный файл как форму `ФормаСписка` с DynamicList и двумя командами | Закрыт первый реальный слой tail body формы: базовые `Attributes` и `Commands`. Это все еще не полная форма: нет полного набора `ChildItems`, `CommandInterface`, `ListSettings`, всех типов реквизитов и полного round-trip compile обратно в blob. |
| Повторный `mssql-dump-config --database ibcmd_rs_sfc_stage_valyuty_20260627 --file-name 7aadbb67-f93e-43bb-9f53-f14d2c2a347a --file-name 5f91b00f-d8fc-4d63-8486-66339357ab22 --file-name 097b7cbf-cd63-4213-96ae-fc88d1be24d6 --file-name b34b580f-305b-4e22-b601-cfcf4dabaae0 --extract-metadata-xml` после разбора `CommandInterface` формы | Selected dump с владельцами ссылочных команд выгрузил валидный `Catalogs/Валюты/Forms/ФормаСписка/Ext/Form.xml` с `<CommandInterface><NavigationPanel>`: `DataProcessor.ЗагрузкаКурсовВалютЕЦБ.Command.ЗагрузитьКурсыВалютЕЦБ`, `InformationRegister.ОтносительныеКурсыВалют.Command.ЗагрузитьКурсыИзТаблицы`, `CommandGroup=FormNavigationPanelGoTo`, `Index=1` у первой команды, `DefaultVisible=false`, `Visible/xr:Common=false`. Ключевые поля совпадают с исходным `D:\УХА\sfc` фрагментом | Закрыт базовый decompile navigation panel внутри формы. Ограничение: для корректных имен команд нужен индекс UUID по владельцам команд; selected dump только формы без владельцев не может восстановить эти ссылки. Остались полный `ChildItems`, `ListSettings`, все типы реквизитов и полный compile формы обратно в blob. |
| Повторный `mssql-dump-config --database ibcmd_rs_sfc_stage_valyuty_20260627 --file-name 7aadbb67-f93e-43bb-9f53-f14d2c2a347a --file-name 5f91b00f-d8fc-4d63-8486-66339357ab22 --file-name 097b7cbf-cd63-4213-96ae-fc88d1be24d6 --file-name b34b580f-305b-4e22-b601-cfcf4dabaae0 --file-name 9e3fce60-09bb-4d21-af76-3ac4e1a12396 --extract-metadata-xml` после разбора `ChildItems` формы | Selected dump с владельцами всех внешних команд выгрузил валидный `Form.xml`; `form-info` видит дерево элементов формы: `ГруппаПользовательскихНастроек`, `КоманднаяПанель`, popup `Создать`, кнопки локальных/стандартных/внешних команд, popup `ГруппаЗагрузитьКурсыВалют`, таблицу `Валюты -> Список`, колонки `НаименованиеПолное`, `Код`, `Наименование`, `ОтносительныйКурс`, `Ссылка`. Восстановлены `CommandName` для локальных команд, стандартных `Create/Help` и внешних команд при наличии owner metadata; восстановлены `DataPath` таблицы и колонок; unit-тестом добавлен round-trip событий элементов `OnGetDataAtServer`, `OnChange`, `StartChoice`; отдельно добавлен round-trip `Button/Type` для `UsualButton`/`CommandBarButton`/`Hyperlink` и `AdditionSource/Item` + `AdditionSource/Type` для `SearchStringAddition`/`ViewStatusAddition`/`SearchControlAddition` | Закрыт базовый decompile layout tree для подтвержденных элементов (`UsualGroup`, `CommandBar`, `Popup`, `ButtonGroup`, `Button`, `Table`, `InputField`, `LabelField`, search additions), частых событий существующих элементов, типа существующих кнопок и source/type search additions. Это все еще не полный `Form.xml`: не покрыты все типы элементов, расширенные свойства элементов, полный compile обратно в blob. |
| Native `source-diff D:\УХА\sfc <mssql-dump-config output> --path-prefix Catalogs/Валюты` после фикса child event records | Structural diff выбранного native dump показывает 16 left-only файлов из-за намеренно выбранного неполного набора `--file-name`, 0 right-only, 4 byte-different XML, 4 unchanged файла; все BSL/HTML в выбранном наборе совпадают, а `Catalogs/Валюты/Forms/ФормаСписка/Ext/Form.xml` теперь содержит табличное событие `<Event name="OnGetDataAtServer">ВалютыПриПолученииДанныхНаСервере</Event>` внутри элемента `Table`, как в эталонном `sfc` | Закрыт дефект native SQL -> XML decompile, где запись события элемента `{1,<uuid>,"handler",...}` ошибочно попадала в верхний `<Form><Events>` как UUID-событие или пропадала после фильтрации child items. Остаточный byte-diff формы остается из-за неполных свойств элементов, ListSettings/tooltip/context menu/mobile sections и минимального metadata XML. |
| Native `source-diff D:\УХА\sfc <mssql-dump-config output> --path-prefix Catalogs/Валюты` после форматирования `Help.xml` как `ibcmd export` | На том же selected dump `Catalogs/Валюты/Ext/Help.xml` перешел из `different` в `unchanged`: summary стал 16 left-only, 0 right-only, 3 different, 5 unchanged. Генератор native dump пишет BOM, `version="2.21"` и закрывает файл на `</Help>` без финального CRLF, как эталонная выгрузка `sfc`; `ru.html` уже совпадал ранее | Закрыт массовый byte-diff wrapper-файлов `Help.xml` для SQL -> XML выгрузки. Остаточные различия выбранного среза теперь сосредоточены в минимальном `Catalogs/Валюты.xml`, `ФормаСписка.xml` и неполном `Ext/Form.xml`. |
| Native `source-diff D:\УХА\sfc <mssql-dump-config output> --path-prefix Catalogs/Валюты` после извлечения generated types справочника | Selected dump с пятью связанными `--file-name` дает summary 16 left-only, 0 right-only, 2 different, 6 unchanged. `Catalogs/Валюты.xml` теперь содержит `InternalInfo` и 5 `xr:GeneratedType`: `CatalogObject.Валюты`, `CatalogRef.Валюты`, `CatalogSelection.Валюты`, `CatalogList.Валюты`, `CatalogManager.Валюты`; размер native XML вырос до 1 794 bytes. Индекс типов также строит `CatalogSelection`, `CatalogList`, `CatalogManager` для ссылок из других XML | Закрыт еще один слой metadata XML справочников в native SQL -> XML выгрузке. `Catalogs/Валюты.xml` все еще byte-different с эталоном `sfc` из-за неполных свойств справочника, табличных частей, реквизитов, форм/команд и прочих XML-секций, но карта generated types уже восстанавливается из SQL blob, а не подставляется под конкретную конфигурацию. |
| Native `source-diff D:\УХА\sfc <mssql-dump-config output> --path-prefix Catalogs/Валюты` после извлечения базовых свойств справочника | Selected dump дает тот же structural summary 16 left-only, 0 right-only, 2 different, 6 unchanged, но `Catalogs/Валюты.xml` вырос до 3 225 bytes и теперь содержит полный wrapper namespaces/BOM, `<Comment/>`, `Hierarchical=false`, `HierarchyType=HierarchyFoldersAndItems`, `LimitLevelCount=false`, `LevelCount=2`, `FoldersOnTop=true`, `UseStandardCommands=true`, `Owners`, `SubordinationUse=ToItems`, `CodeLength=3`, `DescriptionLength=10`, `CodeType=String`, `CodeAllowedLength=Variable`, `CodeSeries=WholeCatalog`, `CheckUnique=false`, `Autonumbering=false`, `DefaultPresentation=AsDescription` | Закрыт базовый scalar-слой `Properties` для справочников в native SQL -> XML выгрузке. Остаточный byte-diff `Catalogs/Валюты.xml` теперь в основном связан со `StandardAttributes`, реквизитами, табличными частями, формами/командами и прочими `ChildObjects`, то есть следующий слой - child metadata decompile, а не корневые простые поля. |
| Native `source-diff D:\УХА\sfc <mssql-dump-config output> --path-prefix Catalogs/Валюты` после извлечения form refs и presentation-свойств справочника | Selected dump сохраняет summary 16 left-only, 0 right-only, 2 different, 6 unchanged, а `Catalogs/Валюты.xml` вырос до 4 459 bytes. Корневой XML теперь содержит `DefaultListForm=Catalog.Валюты.Form.ФормаСписка`, `DefaultChoiceForm=Catalog.Валюты.Form.ФормаСписка`, `IncludeHelpInContents=true`, пустые auxiliary/default folder form refs, `ObjectPresentation` (`Валюта`/`Currency`) и `Explanation` (`Валюты, используемые при расчетах`/`Available currencies.`). Резолюция форм идет через `FormSourceReference`, чтобы wrapped form metadata не превращалась ошибочно в `IntegrationService.ФормаСписка` | Закрыт слой ссылок на формы и мультиязычных presentation-полей справочника. В selected dump `DefaultObjectForm` остается пустым, потому что `ФормаЭлемента` не включена в выбранный `--file-name` набор; в полном dump она должна резолвиться при наличии metadata row формы. Остаточный разрыв `Catalogs/Валюты.xml` остается в `StandardAttributes`, `InputByString`, data-history flags и `ChildObjects` (`Attribute`, `TabularSection`, `Command`, `Form`). |
| Native `source-diff D:\УХА\sfc <mssql-dump-config output> --path-prefix Catalogs/Валюты` после извлечения input/search/history root-блока справочника | Selected dump сохраняет summary 16 left-only, 0 right-only, 2 different, 6 unchanged, а `Catalogs/Валюты.xml` вырос до 5 398 bytes. Добавлены `Characteristics`, `PredefinedDataUpdate=Auto`, `EditType=InDialog`, `QuickChoice=true`, `ChoiceMode=BothWays`, `InputByString` с `Catalog.Валюты.StandardAttribute.Description` и `Catalog.Валюты.StandardAttribute.Code`, `SearchStringModeOnInputByString=Begin`, `FullTextSearchOnInputByString=DontUse`, `ChoiceDataGetModeOnInputByString=Directly`, а после `Explanation` - `CreateOnInput=DontUse`, `ChoiceHistoryOnInput=Auto`, `DataHistory=DontUse`, оба data-history processing flag как `false` | Закрыт еще один root-level слой metadata XML справочников. Для `InputByString` значения извлекаются из raw blob поля со стандартными отрицательными кодами, а не подставляются по имени `Валюты`; enum/default-флаги пока покрыты подтвержденным default-слоем. Остаточный разрыв корневого XML теперь сосредоточен в `StandardAttributes` и `ChildObjects`. |
| Native `source-diff D:\УХА\sfc <mssql-dump-config output> --path-prefix Catalogs/Валюты` после генерации каркаса `StandardAttributes` справочника | Selected dump сохраняет summary 16 left-only, 0 right-only, 2 different, 6 unchanged, а `Catalogs/Валюты.xml` вырос до 14 650 bytes. Добавлен универсальный блок `StandardAttributes` для `PredefinedDataName`, `Predefined`, `Ref`, `DeletionMark`, `IsFolder`, `Owner`, `Parent`, `Description`, `Code`; для `Owner` выставлены `FillChecking=ShowError`, `FillFromFillingValue=true`, `TypeReductionMode=Deny`, для `Parent` - `FillFromFillingValue=true`, для строкового `Code` - `FillValue` пробелами по `CodeLength` | Закрыт обязательный structural-блок стандартных реквизитов справочника в native SQL -> XML выгрузке. Остаточный byte-diff внутри `StandardAttributes` остается по специфичным `ToolTip`/`Synonym`/`FillValue` для `Description` и `Code`, которые лежат в property bag и требуют отдельного mapping; также остаются `ChildObjects` (`Attribute`, `TabularSection`, `Command`, `Form`). |
| Native `source-diff D:\УХА\sfc <mssql-dump-config output> --path-prefix Catalogs/Валюты` после форматирования metadata XML форм | `Catalogs/Валюты/Forms/ФормаСписка.xml` перешел из `different` в `unchanged`: summary стал 16 left-only, 0 right-only, 2 different, 6 unchanged. Native dump теперь пишет form metadata XML с BOM, полным набором namespace-ов `ibcmd`, `version="2.21"`, `<Comment/>`, `<UseInInterfaceCompatibilityMode>Any</UseInInterfaceCompatibilityMode>` и без финального CRLF | Закрыт byte-diff metadata XML для выбранной managed формы. В selected-срезе остаются два крупных разрыва: неполный корневой `Catalogs/Валюты.xml` и неполный layout body `Forms/ФормаСписка/Ext/Form.xml`. |
| Unit round-trip `Form/Parameters` на подтвержденном shape из `ChartsOfAccounts/Хозрасчетный/Forms/ФормаОписанияСчета` | Реальный body формы содержит параметры в `trailing[1]`: `{0,1,{0,"Счет",{"Pattern",{"#",...}},1}}`. Добавлены тесты на выгрузку `<Parameters><Parameter name="Счет"><Type>cfg:ChartOfAccountsRef.Хозрасчетный</Type><KeyParameter>true</KeyParameter>` и на обратный patch существующего параметра в blob с резолюцией generated type через `source_root` | Закрыт базовый round-trip существующих параметров формы: имя, `Type` (`xs:*`/`cfg:*` через общий type pattern) и `KeyParameter`. Ограничение остается прежним для полного compile формы: новые параметры пока не создаются, а неподдержанные сложные свойства параметров не генерируются. |
| Unit round-trip `Form/Attributes/Settings/ListSettings` на реальном shape из `Catalogs/Валюты/Forms/ФормаСписка` | Реальный body формы хранит DCS-настройки DynamicList как ключи `Filter`, `Order`, `ConditionalAppearance`, `ItemsViewMode`, `ItemsUserSettingID`; сложные секции лежат в `{"#",...,{#base64:<DCS XML>}}`. Добавлены тесты на выгрузку `<ListSettings>` с `dcsset:filter`, `dcsset:order`, `dcsset:conditionalAppearance`, `dcsset:itemsViewMode`, `dcsset:itemsUserSettingID` и на обратный patch существующих DCS base64 payload/string values в blob | Закрыт базовый round-trip `ListSettings` для фильтра, сортировки, условного оформления и item-level setting id/view mode динамического списка. Ограничение: сложные правила отбора/оформления внутри DCS XML пока сведены к `viewMode`/`userSettingID`; прочие DCS settings и создание новых ключей не реализованы. |
...
| `cargo test` | 335 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Формы | Существенно частично | `prepare_form_body_row` читает `Ext/Form.xml` и патчит top-level `Title`, `AutoTitle`, верхние `WindowOpeningMode`/`Group`, `CommandBarLocation`, имя, `HorizontalAlign` и `Autofill` уже существующей top-level `AutoCommandBar`, обработчики уже существующих `Events`, включая UUID-based события по точному идентификатору, существующие `Attributes` (`name`, `MainAttribute`, часть `DynamicList` settings: `ManualQuery`, `DynamicDataRead`, `QueryText`, `MainTable` при наличии `source_root`, `ListSettings` для `filter`/`order`/`conditionalAppearance`/`itemsViewMode`/`itemsUserSettingID`), существующие `Commands` (`Title`, `ToolTip`, `Action`, `CurrentRowUse`, `FunctionalOptions` при наличии `source_root`), существующий `CommandInterface/NavigationPanel` (`Command`, `CommandGroup`, `Index`, `DefaultVisible`, `Visible/xr:Common`) и существующие `ChildItems` (`name`, `Title`, события, `Button/Type`, `AdditionSource/Item` + `AdditionSource/Type` для search additions, часть `Button/CommandName`, включая внешние `Kind.Object.Command.Name` при наличии `source_root`, `Button/DataPath` для `Items.<Table>.CurrentData.<Field>` по `Ссылка` и найденным колонкам таблицы) в base blob; для layout-list `{59,count,uuid,item,...}` может добавить отсутствующий top-level `Button` с типом, заголовком и local/standard/external command ref, а для новой локальной `Form.Command.*` создает согласованную command entry в tail-секции; также создает простые top-level контейнеры `CommandBar`, `UsualGroup`, `Popup`, `ButtonGroup`, `Table`, рекурсивно создает поддержанное дерево вложенных контейнеров/кнопок/таблиц/`InputField`/`LabelField`/search additions и добавляет недостающий `Button`, `InputField`, `LabelField`, search addition, `Table` или контейнер внутрь уже существующего контейнера `22` или таблицы `73`; если есть `Ext/Form/Module.bsl`, дополнительно заменяет текст модуля. Полная компиляция `Ext/Form.xml` пока не реализована. |
...
| Forms | Существенно частично | `extract_form_body_xml` возвращает каркас `Form.xml`, распознает top-level `Title`, `Width`, `Height`, `AutoTitle=false`, `WindowOpeningMode`, подтвержденные варианты верхнего `Group` без смешивания с `CommandBarLocation`, сам `CommandBarLocation`, часть `Events` включая UUID-based события `OnOpen`/`ChoiceProcessing`/`NotificationProcessing`/`OnCreateAtServer`, неизвестные UUID-события с сохранением идентификатора и строковые события документных форм `AfterWrite`/`BeforeWrite`/`OnWriteAtServer`/settings-load/user-settings/variant/fill-check, извлекает top-level `AutoCommandBar` включая `HorizontalAlign` и недефолтный `Autofill=false`, базовые tail-секции `Attributes` для `DynamicList` включая `ListSettings` (`filter`, `order`, `conditionalAppearance`, `itemsViewMode`, `itemsUserSettingID`), `Commands`, `CommandInterface/NavigationPanel`, базовое дерево `ChildItems`, `Button/Type` (`UsualButton`, `CommandBarButton`, `Hyperlink`), `AdditionSource/Item` и `AdditionSource/Type` для search additions (`SearchStringRepresentation`, `ViewStatusRepresentation`, `SearchControl`) и частые события элементов (`OnGetDataAtServer`, `OnChange`, `StartChoice`, `ValueChoice`, `AutoComplete`, `Click`, `Clearing`, `URLProcessing`, `TextEditEnd` и др.); resolver форм поддерживает wrapped form metadata code `13` и `14`; часть вложенных item assets выгружается отдельно. Разбор `{4, layout, module, ...}` вынесен в общий `parse_form_body_blob`, который также используется при загрузочной замене модуля формы. |
...
8. Object-level `AdditionalIndexes.xml` для неподтвержденных семейств. В `sfc` подтверждены и закрыты `Document.*.3` и `AccumulationRegister.*.4`; если такие body-файлы встретятся у других видов метаданных, нужно добавить suffix mapping по фактическому `ConfigDumpInfo`.
...
Короткий вывод на 2026-06-28: текущий `ibcmd-rs` уже может выгружать и загружать значимые срезы конфигурации, но пока не может гарантированно полностью выгрузить произвольную SQL-базу в исходники и затем загрузить эти исходники в новую базу без штатного `ibcmd`.
...
| Модули BSL | 29 818 `.bsl` | Высокая | Высокая для поддержанного whitelist; все 29 818 файлов отмечены поддержанными source-side | Проверить большой apply на полном наборе модулей |
| Управляемые формы | 13 044 `Ext/Form.xml`, 590 МБ XML; 12 216 form modules | Низкая/средняя для частичной выгрузки: есть top-level `Title`, `Width`, `Height`, `AutoTitle=false`, `WindowOpeningMode`, `Group`, `CommandBarLocation`, top-level `AutoCommandBar` с `HorizontalAlign` и `Autofill=false`, часть `Events`, включая неизвестные UUID-события без потери идентификатора, базовые `Attributes`/`Parameters`/`Commands`/`CommandInterface`/`ChildItems`, `Button/Type`, `AdditionSource/Item` + `AdditionSource/Type` для search additions, базовый `ListSettings` (`filter`, `order`, `conditionalAppearance`, `itemsViewMode`, `itemsUserSettingID`), расширенный набор событий элементов, module/assets | Низкая/частичная: `Ext/Form.xml` патчит top-level `Title`, `Width`, `Height`, `AutoTitle`, `WindowOpeningMode`/`Group`, `CommandBarLocation`, имя, `HorizontalAlign` и `Autofill` уже существующей top-level `AutoCommandBar`, обработчики уже существующих `Events`, включая UUID-based события по точному идентификатору, существующие `Attributes` включая базовый `ListSettings` (`filter`, `order`, `conditionalAppearance`, `itemsViewMode`, `itemsUserSettingID`), существующие `Parameters` (`Type`, `KeyParameter`), существующие `Commands` включая `FunctionalOptions`, часть существующего `CommandInterface`, часть существующих `ChildItems` включая `Button/Type`, `AdditionSource/Item` + `AdditionSource/Type` search additions и события, внешние command refs для подтвержденных путей при наличии `source_root`, создание простого top-level `Button`, локальной command entry для `Form.Command.*`, простых top-level контейнеров/таблиц, рекурсивного дерева поддержанных контейнеров/кнопок/таблиц/`InputField`/`LabelField`/search additions и недостающих children в существующем контейнере или таблице, `Ext/Form/Module.bsl` заменяет модуль; полный `Ext/Form.xml` не компилируется | Полный compiler/decompiler всех `ChildItems`, все типы реквизитов, расширенный `ListSettings` с реальными правилами DCS-отборов/оформления сверх `viewMode`/`userSettingID`, создание/удаление attributes/parameters/events/commands/items, расширить резолюцию всех ссылок и вложенные form assets |
...
| Общие картинки | 3 248 объектов, 3 247 `Picture.xml` | Высокая по маршрутизации форматов `zip/png/svg/bmp/gif/jpg/ico` | Высокая для `CommonPictures` и части configuration pictures | Проверка всех форматов `zip/png/svg/bmp/gif/jpg/ico` в SQL e2e |
...
| Корневые assets конфигурации и `AdditionalIndexes` | несколько известных файлов `Ext/*`; 3 object-level `AdditionalIndexes.xml` | Средняя/высокая: `Splash`, `ParentConfigurations`, `HomePageWorkArea`, `MobileClientSignature`, `CommandInterface`, `MainSectionCommandInterface`, `ClientApplicationInterface`, `MainSectionPicture`, `StandaloneConfigurationContent`, `Document.AdditionalIndexes`, `AccumulationRegister.AdditionalIndexes` маршрутизируются | Средняя/высокая: те же root assets и подтвержденные `AdditionalIndexes` готовятся обратно как picture/binary/raw-deflated/command-interface rows | SQL e2e на реальной базе и suffix mapping для `AdditionalIndexes` у других семейств, если они встретятся |
...
| Агент форм | compile/decompile `Ext/Form.xml`, form item assets | Есть `audit-form-sources` baseline: 13 044 форм, 590 МБ XML, 12 216 stageable-by-module, 828 без stageable body; source-load audit больше не держит 295 `Ext/Form/Items` assets как uncovered. Обратный compile уже патчит верхние свойства, существующую `AutoCommandBar`, существующие `Events`, существующие `Attributes` и часть `DynamicList` settings включая `MainTable`, существующие `Commands` включая `FunctionalOptions` при наличии `source_root`, часть существующего `CommandInterface/NavigationPanel` и часть существующих `ChildItems`, включая события элементов, локальные/стандартные/внешние `Button/CommandName`, внешний `CommandInterface/NavigationPanel/Item/Command` при наличии `source_root`, явный `Button/DataPath` по `Ссылка`/колонкам таблицы, распознанные картинки элементов формы, создание простого top-level `Button`, согласованной локальной command entry, простых top-level контейнеров/таблиц, рекурсивного дерева поддержанных контейнеров/кнопок/таблиц/`InputField`/`LabelField`/search additions и недостающих children в существующем контейнере или таблице. Следующий критерий - создание/удаление остальных элементов хотя бы на малой форме, затем расширять до `sfc`. |
