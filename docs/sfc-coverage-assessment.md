...
| `cargo test` | 299 passed | Unit-покрытие текущего кода стабильно после изменений. |
...
| Агент parity-аудита | source coverage, SQL dry-run, batch accounting, dump/load diff | Есть `audit-source-load-coverage` source-side baseline без SQL: 55 789 stage entry files, 59 968 potentially stageable body files, 13 344 known uncovered. Следующий критерий - SQL-backed `mssql-audit-source-parity`, который вызывает подготовку blob-ов без записи и фиксирует missing base blobs/failures/batch plan. |
...
3. Добавить SQL-backed dry-run prepare отчет для загрузки всех исходников `sfc` без записи в SQL: переиспользовать `stage_source_objects` prepare path, но не писать scripts и не выполнять SQL; отчет должен показать prepared metadata rows, body rows, failures, missing base blobs и batch accounting.
