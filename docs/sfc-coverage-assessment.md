...
| XML -> SQL обновление совместимой базы | Средняя готовность на поддержанных классах | `audit-source-load-coverage` видит 55 789 stage entry files и 59 976 potentially stageable body files; smoke apply по `Catalogs/Валюты` прошел через `ConfigSave` и штатный `ibcmd apply` | Расширить SQL-backed parity/apply на несколько семейств объектов и проверить batch accounting на длинной загрузке |
