...
| `Ext/Form.xml` | 13 044 | Добавлен `audit-form-sources`: все 13 044 XML валидно парсятся как XML-структура; 12 216 форм имеют `Ext/Form/Module.bsl`, 12 218 имеют вложенные файлы в `Ext/Form`. Все 13 044 `Form.xml` / 590 173 726 XML bytes остаются неподдержанной полной структурой формы. Выгрузка сейчас формирует каркас формы, распознанные `Events` включая document lifecycle (`AfterWrite`, `BeforeWrite`, `OnWriteAtServer`, `OnLoadDataFromSettingsAtServer`, `FillCheckProcessingAtServer`), settings/user-variant events, неизвестные UUID-события без потери идентификатора, top-level `Title`, top-level `AutoCommandBar` с `HorizontalAlign` и `Autofill=false`, верхние свойства, базовые `Attributes` для `DynamicList`, базовые `Commands`, базовый `CommandInterface/NavigationPanel`, базовое дерево `ChildItems` для подтвержденных элементов layout, частые события элементов (`OnGetDataAtServer`, `OnChange`, `StartChoice`, `ValueChoice`, `AutoComplete`, `Click`, `Clearing` и др.) и часть вложенных assets. Загрузка использует `Ext/Form.xml` для патча top-level `Title`, верхних `WindowOpeningMode`/`Group`, имени, `HorizontalAlign` и `Autofill` уже существующей top-level `AutoCommandBar`, обработчиков уже существующих `Events`, включая UUID-based события по точному идентификатору, существующих `Attributes`, существующих `Commands` включая `FunctionalOptions` при наличии `source_root`, части существующего `CommandInterface/NavigationPanel` и части существующих `ChildItems` включая события в base blob, дополнительно заменяет `Ext/Form/Module.bsl`, если он есть; полный compile структуры формы пока не реализован. |
