...
продвинут с "не начат вслепую" до "8 из ~11 отдельных находок закрыты
семенами, осталось `pointsScale`/`valuesScale`/`seriesScale`" -- см.
раздел "Пункт 4" ниже. GanttChart (пункты 1-2) не тронут. Гейты
(`ws`/`mdm`/`wms`/`sslbase`/`ssl`/`ut`/`uh`) прогнаны целиком, СЛОМАНО=0
на каждом. Коммиты этой волны: `fix(form-chart): decode
realSeriesCount>0 on form Chart attributes`, `fix(form-chart): decode
chartType=Line and splineMode`, плюс некоммиченные на момент записи
`legendPlacement`/`titleAreaPlacement`/show-mode находки (см. git log
текущего worktree).
...
Все восемь пунктов подтверждены `cargo test --lib` (2260 passed / 33
failed, тот же список, что и `$D/fail-base.txt`) и полным прогоном
гейтов (`ws`/`mdm`/`wms`/`sslbase`/`ssl`/`ut`/`uh`) с проверкой разности
exact-множеств против `$D/base789/*.parity.json` -- СЛОМАНО=0 везде;
остаток УТ пока СЧЁТНО тот же (443 = 439 host-dep + оставшиеся реальные,
т.к. `ПроверкаКонтрагента`/GanttChart ещё не закрыты целиком).

**НЕ закрыто (пункт 4 всё ещё открыт, но сильно сузился)**:

* `pointsScale`/`valuesScale`/`seriesScale` -- ТРИ похожих, но не
  идентичных структуры, живущие в тех же трёх "funnel-link"-подобных
  30-членных блоках, что пункт-3 оставил неразобранными как "непричастные"
  (`post[41..43]` в старой нумерации; в form-нумерации это фиксированные
  `t[139]`/`t[140]`/`t[141]`, СМЕЩАЕМЫЕ через `tidx()` как всё после
  списка шкал). ДВА семени поверх `chart-form-4series` control --
  `chart-form-pointsscale` (полный `pointsScale`: `titleArea` default +
  `gridLinesShowMode=Show` + `gridLine(width=1,Dotted)` + `labelColor=
  #B4B4B4`) и `chart-form-pointsscale-min` (тот же элемент, но
  `gridLinesShowMode=DontShow` + `gridLine(width=1,Solid)` -- платформенный
  default -- + `labelColor` ВООБЩЕ ОПУЩЕН из XML) -- вместе с baseline
  (элемент отсутствует) дают ТРИ точки для блока `t(tidx(139))`,
  22 подполя (индексация с 0) в baseline → 23 в обоих `pointsScale`-
  семенах. Раскладка подполей теперь ТОЧНЕЕ:
  - `[0..5)` = `2,0,0,2,{1,0}` -- общая обёртка, КОНСТАНТА во всех трёх.
  - `[5]` = сама `titleArea` (вложенный 12-членный кортеж
    `{1,4,0.5,0.5,<font>,<textColor>,<backColor>,1,<border>,
    <borderColor>,4,2,0}` -- font/textColor/backColor/border/borderColor
    узнаваемы по уже разобранным паттернам `form_chart_*` функций) --
    КОНСТАНТА во всех трёх (`AutoFont`/`auto`/`auto`/border(width1,
    WithoutBorder)/`auto`), т.е. titleArea уже несёт default ДАЖЕ когда
    `pointsScale` отсутствует из XML -- присутствие элемента решается НЕ
    здесь.
  - `[6]` = похоже на трёхзначное состояние `gridLinesShowMode`+
    "элемент присутствует": `"2"` в baseline (элемент отсутствует),
    `"1"` при `DontShow`, `"0"` при `Show`.
  - `[7]` = непричастно, везде `"0"`.
  - `[8]` = "флаг наличия `gridLine`": `"0"` в baseline, `"1"` в ОБОИХ
    семенах с `pointsScale` (т.е. `gridLine` пишется, даже если он
    default-Solid/width1 -- это подтверждает, что `[8]`/`[9]` НЕ зависят
    от значений `gridLine`, только от присутствия `pointsScale`).
  - `[9]` = `gridLine` (line-структура `{4,0,{0},<style>,<width>,0,
    <uuid>,0}`) -- ЭТА позиция ЦЕЛИКОМ ОТСУТСТВУЕТ (не просто default)
    в baseline, вставляется только когда `[6]!="2"`; отсюда сдвиг на 1
    для всех последующих подполей. Код стиля линии: `"1"`=Solid
    (подтверждено `chart-form-pointsscale-min`), `"2"`=предположительно
    Dotted (только ОДНО наблюдение, из `chart-form-pointsscale`, второе
    значение стиля не перепроверено отдельным семенем).
  - `[10]`/`[11]`/`[12]` (после сдвига) -- `{3,4,{0}}`/`{7,3,0,1,100}`/
    `{3,4,{0}}` в ОБОИХ `pointsScale`-семенах ОДИНАКОВО, независимо от
    того, что `labelColor` в одном семени `#B4B4B4`, а в другом опущен
    (`auto`) -- значит `labelColor` НЕ здесь. Byte-паттерн
    `11842740` (=`0xB4B4B4`) для `labelColor=#B4B4B4` найден в подполе
    `[13]` (сравнение с control-выровненным `[12]="2"` в min-варианте
    против `{3,0,{11842740}}` в full-варианте, СДВИНУТО на позицию из-за
    того, что `labelColor` сама по себе, видимо, ЕЩЁ один
    условно-present слот поверх `[9]`-вставки -- НЕ до конца
    разобрано, нужен третий семя, варьирующий ТОЛЬКО `labelColor` при
    фиксированном `gridLinesShowMode`, чтобы отделить её собственный
    presence-флаг от `[6]`/`[8]`).
  - `[10]`/`[11]` сами по себе (font-like `{7,3,0,1,100}` и color-like
    `{3,4,{0}}`, ПОРЯДОК как раз обратный titleArea!) намекают на ВТОРУЮ,
    ещё не названную в XML пару font/color внутри этого блока -- не
    исключено, что это относится к меткам самой шкалы (label font),
    непричастно к `pointsScale`'s пяти задокументированным XML-детям;
    остаётся неразобранным.
  Полная раскладка НЕ дописана в код -- три семени (`chart-form-
  pointsscale`, `chart-form-pointsscale-min`, plus их `-src` деревья)
  сохранены в `$D/seeds/` (не скопированы в фикстуру репозитория, т.к.
  без кода декодер вернёт `None`, а не воспроизведёт XML); следующей
  волне нужно ПОВТОРИТЬ семена через `seed.sh` -- деревья уже готовы
  в `$D/seeds/chart-form-pointsscale{,-min}-src/` -- добавить ТРЕТЬЕ
  семя (только `labelColor` варьируется, `gridLinesShowMode`/`gridLine`
  фиксированы на default) и продолжить оттуда.
* `valuesScale`/`seriesScale` -- по аналогии должны жить в `t(tidx(140))`
  ...
  одним семенем в этой волне. `valuesScale` несёт `showTitle`+`titleArea`+
  `labelFormat` (локализованная строка, не голый цвет); `seriesScale` --
  `titleArea`+`gridLine`+`showInChart`. Оба, вероятно, используют тот же
  "titleArea"-префикс `[0..6)`, но проверить нужно семенами по одному
  элементу, как выше для `pointsScale`.
...
позиции" до "3 похожих, наполовину разведанных структуры
(`pointsScale`/`valuesScale`/`seriesScale`)". Метод семян по одному
элементу поверх `chart-form-4series` полностью себя оправдал (8/8 находок
подтвердились с первого семени, ни одной ложной гипотезы); продолжать им
же. `chart-form-4series-src` (и его копии `chart-form-*-src` с одним
изменённым элементом каждая) -- готовые скелеты, бери любой и меняй ОДИН
элемент. `pointsScale`, `valuesScale`, `seriesScale` -- сравнимая по
объёму работа для отдельной волны, но НЕ таких больших размеров, как
GanttChart ниже.

## Порядок атаки (рекомендация)

1. Пункт 4, остаток -- семя на `seriesScale`/`valuesScale` по отдельности
   поверх `chart-form-4series` (метод см. выше, дерево
   `chart-form-pointsscale-src` уже показывает рецепт: скопировать
   `chart-form-4series-src`, добавить ОДИН элемент в нужную XML-позицию
   между `pointsAxis` и `legendPlacement`). Затем дописать полную
   раскладку всех трёх блоков (`t(tidx(139/140/141))`) в
   `format_form_chart_settings_xml`, используя уже найденный общий
   "titleArea"-префикс `[0..6)` как отправную точку.
