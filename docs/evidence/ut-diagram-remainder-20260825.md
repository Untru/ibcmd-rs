...
* `pointsScale`/`valuesScale`/`seriesScale` -- ТРИ похожих, но не
  ...
  списка шкал). Семя `chart-form-pointsscale` (`chart-form-4series`
  control + ТОЛЬКО `<d4p1:pointsScale>` с `titleArea`+
  `gridLinesShowMode=Show`+`gridLine(width=1,Dotted)`+`labelColor=
  #B4B4B4`) доказывает: это блок `t(tidx(139))`, 22 подполя в baseline
  → 23 подполя когда `pointsScale` задан (подполя `[0..6)` -- общий с
  baseline "titleArea"-подобный префикс, ВСЕГДА одинаковый, даже когда
  элемент отсутствует из XML -- т.е. titleArea's font/textColor/
  backColor/border/borderColor уже несут default-паттерн `AutoFont`/
  `auto`/`auto`/`WithoutBorder,1`/`auto` независимо от присутствия
  элемента; ПОЯВЛЕНИЕ элемента в XML решает что-то в подполях `[6..]`).
  Новое в подполе `[9]` (line-подобная структура) -- код стиля линии
  `"2"` вместо привычного `"1"` (Solid) -- вероятно `Dotted`,
  НЕ подтверждено вторым семенем. `labelColor=#B4B4B4` = raw `11842740`
  найден в подполе `[12]` (control-варианте -- `[13]`, сдвиг на 1 из-за
  вставки line-структуры). Полная раскладка НЕ дописана в код --
  сырые находки сохранены в `$D/seeds/chart-form-pointsscale/` (не
  скопированы в фикстуру репозитория, т.к. без кода декодер вернёт
  `None`, а не воспроизведёт XML; следующей волне нужно ПОВТОРИТЬ семя
  через `seed.sh` -- дерево уже готово в
  `$D/seeds/chart-form-pointsscale-src/` -- и продолжить оттуда).
