...
Семейство `--raw-asset` сжимает переданные байты ровно один раз. Для тел,
которые компилятор уже выдал как одиночный raw-deflate-поток (например, тела
шаблонов СКД от offline-компилятора), предназначено семейство
`--compiled-asset`: байты файла проходят fail-closed проверку строгим
декодером payload-слоя `ibcmd-cf` (обязательный `StreamEnd` и полное
потребление входа) и записываются как финальный физический payload дословно,
без повторного сжатия. Простой (несжатый) вход отклоняется диагностикой
`invalid_compiled_asset` с подсказкой использовать `--raw-asset`; это
закрывает ловушку двухслойной компрессии, при которой платформа принимает
запись, но её экспорт падает с «Stream format error»
(см. `docs/evidence/dcs-style-link-probes-2214-20260814.md`).
