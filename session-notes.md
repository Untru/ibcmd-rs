...
- За два месяца проект сделал большой общий шаг, но последний этап почти остановился: полный УТ улучшился лишь с 3 795 до 3 684 несовпадающих файлов, а последний полный БСП — с 1 652 до 1 651.
- Основной цикл «локальный diff → гипотеза → новая ветка в парсере» исчерпал себя: он смешивает декодирование SQL-блобов, восстановление модели и XML-сериализацию.
- 230 тыс. строк Rust и крупные монолиты (`module_blob.rs`, `mssql_dump/mod.rs`, `form_body.rs`) превращают локальные исправления в трудно обозримую систему правил.
- Анализ EDT от 30 июня правильно предлагал использовать его как эталон поведения, но это решение не было превращено в основной рабочий процесс.
- Скомпилированные writer-классы EDT доступны для анализа через bytecode и содержат точные правила порядка, default-значений, версий и делегирования; они уже объясняют ChoiceList, ListSettings и SpreadsheetContent.
- Модели не являются главным ограничением: при недоопределённой локальной задаче и дорогой, и дешёвый агент одинаково вынужден угадывать.
- EDT следует использовать как лабораторный oracle и источник генерируемых таблиц, а не добавлять как обязательную production-зависимость.
- Следующий этап должен быть schema-first: отдельная каноническая модель, независимые слои DB decoder и XML writer, трёхсторонние проверки IBCMD/EDT/ibcmd-rs.
- Новые эвристики допустимы только при наличии правила EDT либо полного корпуса контрпримеров и измеримого выигрыша на полном gate.

## Retrospective — 2026-07-27

- Архитектурный разворот оказался полезным: за два дня сделано 84 коммита, добавлены автономный корпус моделей EDT, типизированные декодеры, owner graph, physical-adapter gate и трёхсторонний oracle.
- Реальные строгие прогоны стали давать воспроизводимые доказательства: фиксируются commit и hash бинарника, отпечатки базы и неизменный нативный эталон из 49 623 файлов УТ.
- Последовательные production-блокеры действительно сдвигаются: последний полный прогон прошёл прежние формы и остановился на новом варианте `ChoiceParameterLink` с терминалом `{-3}`; его профиль уже воспроизведён и исправлен локально.
- По конечной цели эффективность пока недостаточна: полного candidate-export, полного diff УТ/БСП и результата 0/0/0 всё ещё нет; свежий общий baseline после текущих изменений также не получен.
- Цикл «один строгий прогон — один следующий opaque blocker» слишком дорог: перед новым полным прогоном нужно собирать все отказы диагностическим режимом, кластеризовать их по raw-профилям и исправлять пакетами.
- Локальный exact-BCP тест перед полным прогоном существенно экономит время и должен стать обязательным gate для каждого нового production-профиля.
- Долг тестов сокращается медленно: последний сопоставимый baseline улучшился примерно со 100 до 97 падений библиотеки, поэтому миграцию fixture-кластеров нужно вести отдельным потоком, не смешивая с live parity.
- Корпус EDT пока в основном инфраструктурный: типизировано 3 из 4 966 извлечённых feature mappings, поэтому большая часть семантической миграции writer-правил ещё впереди.
- Следующий рабочий ритм: поток A — пакетное снятие source-asset блокеров через сохранённые BCP; поток B — один законченный кластер тестов или одно семейство EDT за итерацию; полные immutable-прогоны только после локальных gates.
- Универсальное правило для `CLAUDE.md`: не запускать полный parity-run, пока exact raw fixture, production extraction на сохранённом BCP, policy gate и целевые тесты не проходят локально; прогресс README менять только по завершённому immutable manifest.
