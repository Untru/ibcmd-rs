...
CF-001 is complete and the CF parser chain is unblocked. The checked-in
Format15/Format16 corpus is hand-authored clean-room data stored as reviewable
Base64, with exact coordinates, decoded SHA-256 hashes, a consistent
`root`/`version`/`versions` graph, and no application code. Format16 uses a
semantic Format15 envelope at exact offset `0x1359`, not a copied opaque byte
constant. An opt-in, network-free external-corpus lane pins independently
published Format15 and Format16 files by repository commit, path, size, hash,
and license without vendoring their application content. See the
[offline CF fixture corpus](tests/fixtures/cf/README.md).

CF-002 moves Format15 structural reads into the portable `ibcmd-v8` crate.
...
CF-004 adds unified Format15/Format16 detection and the native 64-bit reader.
Complete Format16 artifacts are recognized by the independent sentinel at the
standard `0x1359` base after their Format15 envelope. The reader validates and
retains that semantic preamble, then preserves 20-byte file headers, 55-byte
block headers, 24-byte address records, relative 64-bit page addresses, entry
...
CF-012 connects a decoded `StorageImage` to the existing source-family
decoders without introducing SQL, 1C, EDT, Java, or subprocess dependencies.
Multipart physical entries are grouped in stable source order and presented at
one provider-neutral boundary; the legacy MSSQL fetch path remains separate
and keeps its fail-fast behavior. `cf export` emits hierarchical XML for known
families and a versioned per-entry JSON report with `supported`, `opaque`, and
`failed` dispositions. Unknown records are retained as explicit opaque results
instead of being guessed or silently dropped. The checked-in clean-room CF and
a complete synthetic Configuration record both export with an empty `PATH`:
